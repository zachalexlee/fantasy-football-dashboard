"use client";

// Interactive league-history explorer: season + manager filters up top, and a
// sortable franchise table. All data arrives precomputed-ready from the server
// page; filtering re-runs the pure helpers in lib/stats client-side.

import { useMemo, useState } from "react";
import TeamMark from "./TeamMark";
import { fmtPts, ordinal, record } from "@/lib/format";
import {
  championshipHistory,
  franchiseH2H,
  franchises,
  recordBookAllTime,
  seasonBests,
  winPct,
  type AllTimeGame,
  type AllTimePerf,
  type Franchise,
} from "@/lib/stats";
import type { SeasonSlice } from "@/lib/types";

type SortCol = "team" | "seasons" | "titles" | "runnerUps" | "best" | "top5" | "playoffs" | "winpct" | "pf" | "pa";

const SORTERS: Record<SortCol, (f: Franchise) => number | string> = {
  team: (f) => f.team.name.toLowerCase(),
  seasons: (f) => f.seasonsPlayed,
  titles: (f) => f.championships,
  runnerUps: (f) => f.runnerUps,
  best: (f) => f.bestFinish?.rank ?? 99,
  top5: (f) => f.top5,
  playoffs: (f) => f.playoffBerths,
  winpct: (f) => winPct(f),
  pf: (f) => f.pointsFor,
  pa: (f) => f.pointsAgainst,
};
// Columns where "best" is the smallest value, so the first click should sort ascending.
const ASC_FIRST = new Set<SortCol>(["team", "best"]);

export default function HistoryView({ seasons }: { seasons: SeasonSlice[] }) {
  const [season, setSeason] = useState<string>("all");
  const [manager, setManager] = useState<string>("all");
  const [sort, setSort] = useState<{ col: SortCol; asc: boolean } | null>(null);

  const allFranchises = useMemo(() => franchises({ seasons }), [seasons]);
  const filteredSeasons = useMemo(
    () => (season === "all" ? seasons : seasons.filter((s) => `${s.league.season}` === season)),
    [seasons, season]
  );
  const only = manager === "all" ? undefined : manager;

  const viewFranchises = useMemo(() => {
    let rows = franchises({ seasons: filteredSeasons }).filter((f) => f.seasonsPlayed > 0 || season === "all");
    if (only) rows = rows.filter((f) => f.key === only);
    if (sort) {
      const get = SORTERS[sort.col];
      rows = [...rows].sort((a, b) => {
        const av = get(a);
        const bv = get(b);
        const cmp = typeof av === "string" ? av.localeCompare(bv as string) : (av as number) - (bv as number);
        return sort.asc ? cmp : -cmp;
      });
    }
    return rows;
  }, [filteredSeasons, only, sort, season]);

  const champs = useMemo(
    () =>
      championshipHistory({ seasons: filteredSeasons }).filter(
        (c) =>
          !only ||
          (c.champion && franchiseOf(allFranchises, c.champion.ownerGuid ?? c.champion.ownerName) === only) ||
          (c.runnerUp && franchiseOf(allFranchises, c.runnerUp.ownerGuid ?? c.runnerUp.ownerName) === only)
      ),
    [filteredSeasons, only, allFranchises]
  );
  const book = useMemo(() => recordBookAllTime({ seasons: filteredSeasons }, only), [filteredSeasons, only]);
  const bests = useMemo(() => seasonBests({ seasons: filteredSeasons }, only), [filteredSeasons, only]);
  const h2h = useMemo(() => franchiseH2H({ seasons: filteredSeasons }), [filteredSeasons]);

  const gridCols = useMemo(
    () => franchises({ seasons: filteredSeasons }).filter((f) => f.seasonsPlayed > 0 || season === "all"),
    [filteredSeasons, season]
  );
  const gridRows = only ? gridCols.filter((f) => f.key === only) : gridCols;

  const filtered = season !== "all" || manager !== "all";
  const spanYears = seasons.map((s) => s.league.season);
  const spanLabel =
    season !== "all"
      ? season
      : spanYears.length > 1
        ? `${Math.min(...spanYears)}–${Math.max(...spanYears)}`
        : `${spanYears[0] ?? ""}`;

  const header = (label: string, col: SortCol, extra = "") => {
    const active = sort?.col === col;
    return (
      <th className={`px-2 py-2.5 ${extra}`}>
        <button
          onClick={() =>
            setSort(active ? { col, asc: !sort!.asc } : { col, asc: ASC_FIRST.has(col) })
          }
          className={`inline-flex items-center gap-0.5 font-semibold hover:text-ink ${active ? "text-accent" : ""}`}
          title={`Sort by ${label}`}
        >
          {label}
          <span className="w-2.5 text-[9px]">{active ? (sort!.asc ? "▲" : "▼") : ""}</span>
        </button>
      </th>
    );
  };

  return (
    <div className="space-y-10">
      <div>
        <h2 className="mb-1 text-lg font-extrabold tracking-tight">
          League history <span className="text-muted">· {spanLabel}</span>
        </h2>
        <p className="text-sm text-ink2">
          {seasons.length} synced {seasons.length === 1 ? "season" : "seasons"} of championships,
          franchise records, and all-time head-to-heads.
        </p>
      </div>

      {/* Filter row */}
      <div className="flex flex-wrap items-end gap-3">
        <label className="text-xs text-muted">
          <span className="kicker mb-1 block">Season</span>
          <select
            value={season}
            onChange={(e) => setSeason(e.target.value)}
            className="h-9 rounded-lg border border-hairline bg-surface px-2.5 text-sm font-semibold text-ink"
          >
            <option value="all">All seasons</option>
            {seasons.map((s) => (
              <option key={s.league.season} value={s.league.season}>
                {s.league.season}
              </option>
            ))}
          </select>
        </label>
        <label className="text-xs text-muted">
          <span className="kicker mb-1 block">Manager</span>
          <select
            value={manager}
            onChange={(e) => setManager(e.target.value)}
            className="h-9 max-w-64 rounded-lg border border-hairline bg-surface px-2.5 text-sm font-semibold text-ink"
          >
            <option value="all">All managers</option>
            {allFranchises.map((f) => (
              <option key={f.key} value={f.key}>
                {f.team.name} — {f.managerName}
              </option>
            ))}
          </select>
        </label>
        {filtered && (
          <button
            onClick={() => {
              setSeason("all");
              setManager("all");
            }}
            className="h-9 rounded-lg px-3 text-sm font-semibold text-accent hover:bg-surface2"
          >
            Reset
          </button>
        )}
      </div>

      <section>
        <h3 className="mb-3 font-bold">
          Franchise records {season === "all" ? "(all-time)" : `(${season})`}
        </h3>
        <div className="card table-scroll">
          <table className="w-full min-w-190 text-sm">
            <thead>
              <tr className="border-b border-hairline text-left text-xs text-muted">
                {header("Manager", "team", "pl-3")}
                {header("Seasons", "seasons", "text-center")}
                {header("🏆 Titles", "titles", "text-center")}
                {header("🥈", "runnerUps", "text-center")}
                {header("Best finish", "best")}
                {header("Top-5s", "top5", "text-center")}
                {header("Playoffs", "playoffs", "text-center")}
                <th className="px-2 py-2.5">W-L-T</th>
                {header("Win %", "winpct", "text-right")}
                {header("Total PF", "pf", "text-right")}
                {header("Total PA", "pa", "pr-3 text-right")}
              </tr>
            </thead>
            <tbody>
              {viewFranchises.map((f) => (
                <tr
                  key={f.key}
                  className={`border-b border-hairline last:border-0 ${f.inCurrentSeason ? "" : "text-ink2"}`}
                >
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      <TeamMark team={f.team} size="sm" />
                      <div className="min-w-0">
                        <div className="max-w-44 truncate font-semibold">{f.team.name}</div>
                        <div className="truncate text-xs text-muted">
                          {f.managerName}
                          {!f.inCurrentSeason && " · alumni"}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-2 py-2 text-center tnum">{f.seasonsPlayed}</td>
                  <td className="px-2 py-2 text-center tnum font-bold">
                    {f.championships > 0
                      ? `${"🏆".repeat(Math.min(f.championships, 3))}${f.championships > 3 ? ` ×${f.championships}` : ""}`
                      : "—"}
                  </td>
                  <td className="px-2 py-2 text-center tnum text-ink2">{f.runnerUps || "—"}</td>
                  <td className="px-2 py-2 tnum">
                    {f.bestFinish
                      ? `${ordinal(f.bestFinish.rank)} ('${`${f.bestFinish.season}`.slice(2)})`
                      : "—"}
                  </td>
                  <td className="px-2 py-2 text-center tnum">{f.top5 || "—"}</td>
                  <td className="px-2 py-2 text-center tnum">{f.playoffBerths || "—"}</td>
                  <td className="px-2 py-2 tnum">{record(f.wins, f.losses, f.ties)}</td>
                  <td className="px-2 py-2 text-right tnum font-semibold">
                    {(winPct(f) * 100).toFixed(1)}%
                  </td>
                  <td className="px-2 py-2 text-right tnum">{fmtPts(f.pointsFor, 0)}</td>
                  <td className="px-3 py-2 text-right tnum text-ink2">{fmtPts(f.pointsAgainst, 0)}</td>
                </tr>
              ))}
              {!viewFranchises.length && (
                <tr>
                  <td colSpan={11} className="px-3 py-3 text-sm text-muted">
                    No seasons match this filter.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
        <p className="mt-2 text-xs text-muted">
          Click a column to sort. Managers matched across seasons by ESPN account; best finish
          counts completed seasons, totals include the season in progress.
        </p>
      </section>

      {champs.length > 0 && (
        <section>
          <h3 className="mb-3 font-bold">Championship history</h3>
          <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {champs.map((c) => (
              <li key={c.season} className="card p-4">
                <div className="kicker text-accent">{c.season}</div>
                {c.champion && (
                  <div className="mt-1.5 flex items-center gap-2">
                    <TeamMark team={c.champion} size="md" />
                    <div className="min-w-0">
                      <div className="truncate font-bold">🏆 {c.champion.name}</div>
                      <div className="truncate text-xs text-muted">{c.champion.ownerName}</div>
                    </div>
                  </div>
                )}
                {c.runnerUp && (
                  <div className="mt-2 truncate text-xs text-ink2">
                    🥈 {c.runnerUp.name} <span className="text-muted">({c.runnerUp.ownerName})</span>
                  </div>
                )}
              </li>
            ))}
          </ul>
        </section>
      )}

      <div className="grid gap-8 lg:grid-cols-2">
        <ScoreList title="🚀 Highest single-week scores" rows={book.topScores} />
        <ScoreList title="🥶 Lowest single-week scores" rows={book.lowScores} />
        <GameList title="💥 Biggest blowouts" rows={book.blowouts} />
        <GameList title="🫀 Closest finishes" rows={book.nailbiters} />
      </div>

      <div className="grid gap-8 lg:grid-cols-2">
        <section>
          <h3 className="mb-3 font-bold">🔥 Best scoring seasons</h3>
          <ol className="card divide-y divide-hairline">
            {bests.topPF.map((r, i) => (
              <li key={i} className="flex items-center gap-3 px-3 py-2 text-sm">
                <span className="tnum w-5 text-center text-xs font-bold text-muted">{i + 1}</span>
                <div className="min-w-0 flex-1">
                  <span className="font-semibold">{r.team.name}</span>
                  <span className="ml-1.5 text-xs text-muted">
                    {r.season} · {record(r.record.wins, r.record.losses, r.record.ties)}
                  </span>
                </div>
                <span className="tnum font-bold">{fmtPts(r.pf, 0)}</span>
              </li>
            ))}
            {!bests.topPF.length && (
              <li className="px-3 py-2.5 text-sm text-muted">No completed seasons match.</li>
            )}
          </ol>
        </section>
        <section>
          <h3 className="mb-3 font-bold">📈 Best season records</h3>
          <ol className="card divide-y divide-hairline">
            {bests.bestRecords.map((r, i) => (
              <li key={i} className="flex items-center gap-3 px-3 py-2 text-sm">
                <span className="tnum w-5 text-center text-xs font-bold text-muted">{i + 1}</span>
                <div className="min-w-0 flex-1">
                  <span className="font-semibold">{r.team.name}</span>
                  <span className="ml-1.5 text-xs text-muted">
                    {r.season} · {fmtPts(r.pf, 0)} PF
                  </span>
                </div>
                <span className="tnum font-bold">
                  {record(r.record.wins, r.record.losses, r.record.ties)}
                </span>
              </li>
            ))}
            {!bests.bestRecords.length && (
              <li className="px-3 py-2.5 text-sm text-muted">No completed seasons match.</li>
            )}
          </ol>
        </section>
      </div>

      <section>
        <h3 className="mb-2 font-bold">Head-to-head</h3>
        <p className="mb-3 text-xs text-muted">
          Row manager&apos;s record against column manager
          {season === "all" ? ", every season combined" : ` in ${season}`} (playoffs included).
        </p>
        <div className="card table-scroll">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-hairline text-muted">
                <th className="px-2 py-2 text-left">vs →</th>
                {gridCols.map((f) => (
                  <th key={f.key} className="px-1.5 py-2 text-center font-semibold">
                    {f.team.abbrev}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {gridRows.map((a) => (
                <tr key={a.key} className="border-b border-hairline last:border-0">
                  <td className="flex items-center gap-1.5 px-2 py-1.5 font-semibold">
                    <TeamMark team={a.team} size="sm" />
                    <span className="hidden sm:inline">{a.team.abbrev}</span>
                  </td>
                  {gridCols.map((b) => {
                    if (a.key === b.key)
                      return (
                        <td key={b.key} className="bg-surface2 px-1.5 py-1.5 text-center text-muted">
                          ·
                        </td>
                      );
                    const c = h2h(a.key, b.key);
                    const played = c.w + c.l + c.t > 0;
                    return (
                      <td
                        key={b.key}
                        className={`px-1.5 py-1.5 text-center tnum ${!played ? "text-muted" : c.w > c.l ? "font-semibold text-goodtext" : c.l > c.w ? "text-bad" : ""}`}
                      >
                        {played ? `${c.w}-${c.l}${c.t ? `-${c.t}` : ""}` : "—"}
                      </td>
                    );
                  })}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </section>
    </div>
  );
}

function franchiseOf(all: Franchise[], guidOrName: string | null): string | undefined {
  if (!guidOrName) return undefined;
  return all.find((f) => f.key === guidOrName || f.managerName === guidOrName)?.key;
}

function ScoreList({ title, rows }: { title: string; rows: AllTimePerf[] }) {
  return (
    <section>
      <h3 className="mb-3 font-bold">{title}</h3>
      <ol className="card divide-y divide-hairline">
        {rows.map((r, i) => (
          <li key={i} className="flex items-center gap-3 px-3 py-2 text-sm">
            <span className="tnum w-5 text-center text-xs font-bold text-muted">{i + 1}</span>
            <div className="min-w-0 flex-1">
              <span className="font-semibold">{r.teamName}</span>
              <span className="ml-1.5 text-xs text-muted">
                Wk {r.week} &rsquo;{`${r.season}`.slice(2)} vs {r.oppAbbrev}
              </span>
            </div>
            <span className="tnum font-bold">{fmtPts(r.points)}</span>
          </li>
        ))}
        {!rows.length && <li className="px-3 py-2.5 text-sm text-muted">No games match this filter.</li>}
      </ol>
    </section>
  );
}

function GameList({ title, rows }: { title: string; rows: AllTimeGame[] }) {
  return (
    <section>
      <h3 className="mb-3 font-bold">{title}</h3>
      <ol className="card divide-y divide-hairline">
        {rows.map((m, i) => (
          <li key={i} className="flex items-center gap-3 px-3 py-2 text-sm">
            <span className="tnum w-16 shrink-0 text-xs text-muted">
              Wk {m.week} &rsquo;{`${m.season}`.slice(2)}
            </span>
            <div className="min-w-0 flex-1 truncate">
              <span className={m.homeWon ? "font-bold" : ""}>{m.homeAbbrev}</span>{" "}
              <span className="tnum">{fmtPts(m.homeScore)}</span>
              <span className="text-muted"> — </span>
              <span className="tnum">{fmtPts(m.awayScore)}</span>{" "}
              <span className={!m.homeWon ? "font-bold" : ""}>{m.awayAbbrev}</span>
            </div>
            <span className="tnum text-xs font-semibold text-ink2">Δ {fmtPts(m.margin)}</span>
          </li>
        ))}
        {!rows.length && <li className="px-3 py-2.5 text-sm text-muted">No games match this filter.</li>}
      </ol>
    </section>
  );
}
